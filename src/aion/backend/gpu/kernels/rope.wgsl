...
// f16 twin: the angle, the sincos and the rotation are all f32, exactly as the
// CPU f16 path computes them; only the load widens and the store rounds. `pos`
// is i32 for both, so it keeps its own binding.
...
@compute @workgroup_size(64)
fn rope_f16(@builtin(global_invocation_id) g: vec3<u32>, @builtin(num_workgroups) nwg: vec3<u32>) {
    let step = nwg.x * 64u;
    for (var idx = g.x; idx < p.count; idx += step) {
        let h = idx % p.th;
        let row = idx / p.th;
        ...
        let base = row * p.th;
        let xl = f32(xh[base + i]);
        let xr = f32(xh[base + p.pairs_total + i]);
        if (h < p.pairs_total) {
            oh[idx] = f16(xl * cs - xr * s);
        } else {
            oh[idx] = f16(xl * s + xr * cs);
        }
    }
}
