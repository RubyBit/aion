...
pub fn matmulF32(params: MatMulParams, c_bytes: []u8, a_bytes: []const u8, b_bytes: []const u8) BackendError!void {
    ...
    for (0..m) |i| {
        ...
        while (j + lanes <= n) : (j += lanes) {
            ...
            var kk: usize = 0;
            while (kk < k) : (kk += 1) {
                const a_s: f32 = a[i * k + kk];
                const a_v: Vec = @splat(a_s);
                const b_v: Vec = @as(*align(1) const Vec, @ptrCast(b.ptr + kk * n + j)).*;
                acc += a_v * b_v;
            }

            const c_idx: usize = i * n + j;
            const c_old: Vec = @as(*align(1) const Vec, @ptrCast(c.ptr + c_idx)).*;
            const out_v: Vec = alpha_v * acc + beta_v * c_old;
            @as(*align(1) Vec, @ptrCast(c.ptr + c_idx)).* = out_v;
        }
        ...
        while (j < n) : (j += 1) {
            ...
            var kk: usize = 0;
            while (kk < k) : (kk += 1) {
                acc_s += a[i * k + kk] * b[kk * n + j];
            }
            const c_idx: usize = i * n + j;
            c[c_idx] = alpha * acc_s + beta * c[c_idx];
        }
    }
}
