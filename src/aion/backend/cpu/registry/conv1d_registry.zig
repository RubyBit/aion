...
pub const candidates = [_]Candidate{
    .{ .id = .baseline, .kernels = kernelsFor(.{ .unroll_k3 = false, .lanes = 4 }) },
    .{ .id = .avx2, .kernels = kernelsFor(.{ .unroll_k3 = true, .lanes = 8 }) },
    .{ .id = .avx512, .kernels = kernelsFor(.{ .unroll_k3 = true, .lanes = 16 }) },
};
