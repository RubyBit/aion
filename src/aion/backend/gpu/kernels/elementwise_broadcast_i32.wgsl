...
fn divide(@builtin(global_invocation_id) g: vec3<u32>) {
    ...
    if (divisor == 0) {
        ...
        o[g.x] = a[ix.x] / divisor;
    }
}
@compute @workgroup_size(64)
fn eq(@builtin(global_invocation_id) g: vec3<u32>) {
    if (g.x >= p.n) { return; }
    let ix = indices(g.x); o[g.x] = select(0, 1, a[ix.x] == b[ix.y]);
}
@compute @workgroup_size(64)
fn ne(@builtin(global_invocation_id) g: vec3<u32>) {
    if (g.x >= p.n) { return; }
    let ix = indices(g.x); o[g.x] = select(0, 1, a[ix.x] != b[ix.y]);
}
@compute @workgroup_size(64)
fn lt(@builtin(global_invocation_id) g: vec3<u32>) {
    if (g.x >= p.n) { return; }
    let ix = indices(g.x); o[g.x] = select(0, 1, a[ix.x] < b[ix.y]);
}
@compute @workgroup_size(64)
fn gt(@builtin(global_invocation_id) g: vec3<u32>) {
    if (g.x >= p.n) { return; }
    let ix = indices(g.x); o[g.x] = select(0, 1, a[ix.x] > b[ix.y]);
}
@compute @workgroup_size(64)
fn le(@builtin(global_invocation_id) g: vec3<u32>) {
    if (g.x >= p.n) { return; }
    let ix = indices(g.x); o[g.x] = select(0, 1, a[ix.x] <= b[ix.y]);
}
@compute @workgroup_size(64)
fn ge(@builtin(global_invocation_id) g: vec3<u32>) {
    if (g.x >= p.n) { return; }
    let ix = indices(g.x); o[g.x] = select(0, 1, a[ix.x] >= b[ix.y]);
}
